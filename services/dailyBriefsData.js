/**
 * Daily Mumbai & MMR Redevelopment Intelligence Briefs Data Service
 * Maintains all historical daily briefs (July 8, 2026 - July 21, 2026 and dynamic daily updates)
 */
...
const INITIAL_DAILY_BRIEFS = [
  ...
  },
  {
    ...
    publishedAt: new Date('2026-07-18T08:00:00.000Z'),
    title: 'Daily Mumbai & MMR Redevelopment Intelligence Brief',
...
async function seedInitialDailyBriefs() {
  try {
    const count = await DailyBrief.countDocuments();
    if (count === 0) {
      console.log('🌱 Seeding initial Daily Intelligence Briefs...');
      await DailyBrief.insertMany(INITIAL_DAILY_BRIEFS);
      console.log(`✅ Successfully seeded ${INITIAL_DAILY_BRIEFS.length} Daily Intelligence Briefs.`);
    }
  } catch (error) {
    console.error('Error seeding Daily Intelligence Briefs:', error.message);
  }
}

async function getDailyBriefs({ page = 1, limit = 12, category, search } = {}) {
  try {
    let briefs = [];
    ...
    if (category && category !== 'All' && category !== 'Daily Briefs') {
      query.$or = [
        { focus: new RegExp(category, 'i') },
        { 'rankedItems.category': new RegExp(category, 'i') }
      ];
    }
    ...
    if (search) {
      ...
      query.$or = [
        ...
        { 'rankedItems.title': searchRegex },
        { 'rankedItems.whatHappened': searchRegex },
        { 'rankedItems.whyItMatters': searchRegex },
        { 'rankedItems.dprImplication': searchRegex }
      ];
    }

    const total = await DailyBrief.countDocuments(query);
    briefs = await DailyBrief.find(query)
      .sort({ publishedAt: -1 })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean();

    // Fallback to memory array if DB returned 0 (e.g. offline/no connection)
    if (briefs.length === 0 && !search && (!category || category === 'All' || category === 'Daily Briefs')) {
      briefs = INITIAL_DAILY_BRIEFS;
      return {
        briefs: briefs.slice((page - 1) * limit, page * limit),
        page,
        total: briefs.length,
        totalPages: Math.ceil(briefs.length / limit)
      };
    }
    ...
    console.error('Error fetching DailyBriefs from DB, using fallback memory array:', error.message);
    const filtered = INITIAL_DAILY_BRIEFS.filter(b => {
      if (search) {
        const str = (b.title + ' ' + b.focus + ' ' + (b.executiveSummary || []).join(' ')).toLowerCase();
        return str.includes(search.toLowerCase());
      }
      return true;
    });
    return {
      briefs: filtered.slice((page - 1) * limit, page * limit),
      page,
...
async function getLatestDailyBrief() {
  try {
    const brief = await DailyBrief.findOne({ isPublished: true }).sort({ publishedAt: -1 }).lean();
    if (brief) return brief;
  } catch (error) {
    console.warn('Unable to query latest brief from DB, using memory fallback:', error.message);
  }
  return INITIAL_DAILY_BRIEFS[0];
}

async function getDailyBriefByDate(dateOrId) {
  try {
    const brief = await DailyBrief.findOne({
      $or: [{ briefId: dateOrId }, { date: new RegExp(dateOrId, 'i') }]
    }).lean();
    if (brief) return brief;
  } catch (error) {
    console.warn('Unable to query brief by date from DB:', error.message);
  }
  return INITIAL_DAILY_BRIEFS.find(b => b.briefId === dateOrId || b.date.includes(dateOrId)) || null;
}

module.exports = {
  INITIAL_DAILY_BRIEFS,
  seedInitialDailyBriefs,
  getDailyBriefs,
  getLatestDailyBrief,
  getDailyBriefByDate
};
